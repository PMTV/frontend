...
ORD.controller('ReturnSaleOrderListCtrl', function ($rootScope, $scope, returnSaleOrderService) {
...
PUR.controller('ReturnSaleOrderEditCtrl', function ($rootScope, $scope, $routeParams, returnSaleOrderService, supplierInvoiceService, creditNoteApAgainstInvoiceService, $location, $modal, $translate, $filter, $sce, errorDisplay) {
    $scope.submitted = false;
    $scope.busy = false;
    $scope.checkEdit = false;
    $scope.returnSaleOrder = {};
    $scope.temp = null;

    console.info('Stopped - TODO!');
    return false;

    var returnSaleOrderId = $routeParams.id;

    $scope.onChangedCustomer = function (id) {
        if(id>0){
            returnSaleOrderService.getSaleOrderByCustomerId(id)
                .success(function (data, status) {
                    $scope.returnSaleOrder.salesOrderList = data;
                })
                .error(function (data, status) {
                    alert('Error load list sale order!');
                })
                .finally();
        }
    };
